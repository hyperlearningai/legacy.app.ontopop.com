...
const setOntologyDeleteNode = async ({
  ...
  for (let index = selectedElementLength; index >= 0; index--) {
    const invertedIndex = selectedElementLength - index
    const nodeId = selectedElement[invertedIndex]
    ...
    if (edgeIds) {
      const edgeIdsLength = edgeIds.length - 1

      // on each selected node, first remove connection then remove node
      for (let edgeIndex = edgeIdsLength; edgeIndex >= 0; edgeIndex--) {
        const edgeId = edgeIds[edgeIdsLength - edgeIndex]
    ...
    if (nodesToRestyle.length > 0 && invertedIndex === selectedElementLength) {
      const visibleEdges = getEdgeIds()

      const nodesToRestyleLength = nodesToRestyle.length - 1

      // on each selected node, first remove connection then remove node
      for (let nodeIndex = nodesToRestyleLength; nodeIndex >= 0; nodeIndex--) {
        const nodeToRestyleId = nodesToRestyle[nodesToRestyleLength - nodeIndex]
